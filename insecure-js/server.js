      ...
      try {
          ...
          asyncTasks.push(
            (async () => {
              try {
                ...
                const result = await sequelize.query(query, { replacements: { orderNumber: postData.orderNumber }, type: sequelize.QueryTypes.SELECT });
        ...
        if (postData.orderNumber2) {
          ...
          asyncTasks.push(
            new Promise((resolve) => {
              const query = 'SELECT product FROM Orders WHERE orderNumber = $orderNumber';
              db.all(query, { $orderNumber: postData.orderNumber2 }, (err, rows) => {
                if (err) {
